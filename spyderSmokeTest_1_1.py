...
def getCardData(meterUnderTest):
	...
	if (meterPhases == 2):							#Determines the appropriate channel depending on # of phases
	...
	while (i<16):								#Check Card Data
		if ((i > 5 and i < 9) or (i > 11 and i < 16)):					#Check load, current, voltage, watt, var, va, and pf
			if (value[i] == " "):
				...
				if (theory[i] * high <= value[i] <= theory[i] * low):
					style[i] = 1
				else:
					style[i] = 3
			elif (theory[i] > 0):
				if (theory[i] * low <= value[i] <= theory[i] * high):
					style[i] = 1
				else:
					style[i] = 3	
		i = i + 1	
...
matchValues()						#Compares values against each other
matchValues()						#Compares values against each other
if (checkValuesPlease == 1):		#Checks values against theoreticals
